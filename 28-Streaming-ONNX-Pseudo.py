...
class Qwen3ASRPseudoStreamer:
    def __init__(self):
        print("--- 初始化 ONNX/GGUF 伪流式引擎 ---")
        ...
        self.sampler = llama.LlamaSampler(temperature=0.0) # 贪心解码

    def run_chunk_inference(self, audio_accum, prefix_text=""):
        # A. Mel 提取
        inputs = self.processor(text="语音转录：", audio=audio_accum, return_tensors="pt")
...
def main():
    ...
    print(f"\n--- Starting ONNX Pseudo-Streaming (Chunk: {CHUNK_SIZE_SEC}s) ---")
    ...
    for i in range(total_chunks):
        ...
        print(f"\n[{i+1}/{total_chunks}] {CHUNK_SIZE_SEC*(i+1):.1f}s | Prefix: '{prefix_text}'")
        print("Output: ", end="", flush=True)
        
        new_text, all_tokens = engine.run_chunk_inference(audio_accum, prefix_text)
        last_all_tokens = all_tokens
        
        print(f"\nCost: {time.time() - t_start:.2f}s")
